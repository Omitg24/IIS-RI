...
public class FindSteps {
	...
	private Optional<ContractTypeBLDto> optionalFound;
	private ContractTypeBLDto dto = null;
	private List<ContractTypeBLDto> allFound = null;

	public FindSteps(TestContext ctx) {
		this.ctx = ctx;
	}

	@When("I try to find a contract type with null id")
	public void i_try_to_find_a_contract_type_with_null_id() {
		tryFindAndKeepException(null);
	}

	@When("I try to find a contract type with arg {string}")
	...
	public void contract_type_permanent_is_found() {
		...
		finder.execute();
		ContractTypeBLDto dto = finder.get().get();
