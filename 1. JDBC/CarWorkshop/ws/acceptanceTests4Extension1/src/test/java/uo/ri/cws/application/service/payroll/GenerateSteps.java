...
public class GenerateSteps {
	...
	public void one_payroll_is_generated_with(Double moneyMonth, Double bonus, Double productivity, Double tri,
	...
	public void payrolls_are_generated_with(Integer howMany, Double moneyMonth, Double bonus, Double productivity,
			Double tri, Double tax, Double nic) {
		List<PayrollBLDto> founds = findGeneratedPayrolls();
		this.generatedPayslips = founds.size();
	...
	private List<PayrollBLDto> findGeneratedPayrolls() {
		...
		FindPayrollsThisMonthSqlUnitOfWork 
			unit = new FindPayrollsThisMonthSqlUnitOfWork(currentDate);
		unit.execute();
	...
	public void workorders_for_the_mechanic_with_date_and_amount(Integer howmany, 
			Double amount) {
		 ...
		 */
		ClientBLDto client = new ClientUtil().register().get();
		VehicleBLDto vehicle = new VehicleUtil().withOwner(client.id).register().get();
		for (int i = 1; i <= howmany; i++) {
			LocalDateTime ldt = openWorkorder
					.withDayOfMonth(i)
					.atStartOfDay();
					
			new WorkOrderUtil().forMechanic(m.id)
	...
	public void a_contract_in_force_for_the_mechanic_with
		( String date, Double salary) throws BusinessException {
		MechanicBLDto m = new MechanicUtil().unique().register().get();
		ctx.put(Key.MECHANIC, m);

		ContractTypeBLDto ct = new ContractTypeUtil()
	...
	public void a_contract_terminated_for_the_mechanic_with_start_date_end_date_base_salary(
			String startDate, String endDate, Double salary) throws BusinessException {
		MechanicBLDto m = new MechanicUtil().unique().register().get();
		ctx.put(Key.MECHANIC, m);
		...
		LocalDate last = LocalDate.parse(endDate, formatter).with(TemporalAdjusters.lastDayOfMonth());
	...
	public void a_contract_type_permanent() {
		ContractTypeUtil util = new ContractTypeUtil().findContractType("PERMANENT");
		ContractTypeBLDto ct_dto = util.get();
	...
	public void a_prof_group_1() {
		ProfessionalGroupUtil util = new ProfessionalGroupUtil().findProfessionalGroup("I");
