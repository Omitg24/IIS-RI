...
public class DeleteSteps {
	...
	private ContractTypeService service = BusinessFactory.forContractTypeService();
	private String name2Del = null;
	private ContractTypeBLDto typeDto;
	private ProfessionalGroupBLDto profGroupDto;
	
	public DeleteSteps(TestContext ctx) {
		this.ctx = ctx;
		typeDto = ((ContractTypeBLDto)ctx.get(Key.CONTRACTTYPE));
		profGroupDto = ((ProfessionalGroupBLDto)ctx.get(Key.PROFESSIONALGROUP));
	}
	...
	public void a_contract_in_force_for_the_mechanic_in_contract_type_permanent() throws BusinessException {
		MechanicBLDto m= ((MechanicBLDto)ctx.get(Key.MECHANIC));
	    new ContractUtil().unique().forMechanic(m).withType(typeDto).withGroup(profGroupDto).register().get();
	...
	public void this_contract_type_is_deleted() {
		FindContractTypeByIdSqlUnitOfWork unit = new FindContractTypeByIdSqlUnitOfWork(this.name2Del);
		unit.execute();
	...
	public void i_try_to_del_contract_type_i() {
		String id = null;
		FindContractTypeByNameSqlUnitOfWork unit = new FindContractTypeByNameSqlUnitOfWork("PERMANENT");
		unit.execute();
		id = unit.get().get().id;
