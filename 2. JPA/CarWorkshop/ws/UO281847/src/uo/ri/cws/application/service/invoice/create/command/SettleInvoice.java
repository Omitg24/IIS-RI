...
public class SettleInvoice implements Command<Void> {
	 ...
	 * Atributo charges
	 */
	private Map<Long, Double> charges;
	...
	public Void execute() throws BusinessException {
		...
		for (Long paymentMeanId : charges.keySet()) {
		...
		for (PaymentMean pm : paymentMeans) {
			Factory.repository.forCharge().add(new Charge(i, pm, charges.get(Long.valueOf(pm.getId()))));
		}		
		i.settle();
		return null;
	}
