...
public class InvoiceTests {
	...
	public void testInvoiceAmountAddingTwoWorkOrders() {
		...
		assertEquals(invoice.getAmount(), 468.88, Double.MIN_VALUE); // 2 cents rounding
	}
	...
	/**
	 * A work order, when added to an invoice, changes its status to INVOICED Added
	...
	/**
	 * All the work orders changes its status to INVOICED when added to an invoice
	 */
	@Test
	...
	/**
	 * Creates a new invoiced witha delay of 100 milliseconds to avoid a collision
	 * in the dates field (same millisecond)
	 *
	 * It could be problematic if the identity of the work order depends on the date
	 * 
